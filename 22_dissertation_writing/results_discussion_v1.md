...
*   *Internal Consistency / Cross-Cohort Agreement (Weaker)*: Asserting measurement accuracy because two independent cohorts demonstrate similar kinematic means (such as the squat laboratory-vs-Penn Action cohort agreement in Section 4.3, or the lunge subject-specific shift consistency in Section 5.2.2). While useful for demonstrating reproducibility, cross-cohort consistency cannot detect systematic coordinate offsets or characterize individual trial variance [source: Section 6.1].
