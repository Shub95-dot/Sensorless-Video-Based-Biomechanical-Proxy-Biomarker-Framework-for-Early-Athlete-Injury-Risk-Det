...
*   **#2 peak_landing_flexion [MEASURED]**: Evaluated at the frame of peak landing absorption, where knee angular velocity is near-zero ($\omega \approx 0$). As established in Section 6.2 and Section 6.5.1 of Chapter 6, peak-matching (artificial temporal alignment of the video and motion capture peaks) was rejected because it inflated trajectory MAE (worsening from $14.80^\circ$ to $20.15^\circ$) [source: 22_dissertation_writing/results_dropjump_validation_v1.md]. Instead, the timing-clean peak variance is isolated from the static-peak analysis where joint velocity is approximately zero, ensuring that the total validated variance of $37.4132$ represents the purely spatial projection-based component [source: 17_uncertainty_framework_outputs/framework_design.md]:
