#!/usr/bin/env python3
"""
phase5a_rehab24_integration.py
==============================
Phase 5A — REHAB24-6 Integration (Sagittal Squat Cohort)

Applies the Phase 3 -> 4F pipeline methodology to REHAB24-6's squat recordings
...
def main():
    print("=== Phase 5A — REHAB24-6 Integration Script Started ===")
    project_root = Path(__file__).resolve().parent.parent
    
    # Define relative input paths
    seg_csv_path = project_root / "1_raw_datasets" / "New Dataset for dissertation" / "REHAB24-6 integration" / "Squats" / "Segmentation.csv"
    seg_txt_path = project_root / "1_raw_datasets" / "New Dataset for dissertation" / "REHAB24-6 integration" / "Squats" / "Segmentation.txt"
    video_dir = project_root / "1_raw_datasets" / "New Dataset for dissertation" / "REHAB24-6 integration" / "Squats"
    model_path = project_root / "12_models" / "pose_landmarker_heavy.task"

    # Define output paths
    out_dir = project_root / "14_rehab24_outputs"
    ...
    # 2. Build the sagittal squat manifest (Stage 1)
    print("Loading Segmentation.csv and building sagittal manifest...")
    df_raw = pd.read_csv(seg_csv_path, sep=';')
    
    # Filter for squats (exercise_id == 6), cam17 orientation 'front' (so cam18 is side/sagittal), and mocap_erroneous == 0
    df_filtered = df_raw[
        (df_raw['exercise_id'] == 6) &
    ...
    manifest_cols = [
        ...
        'correctness_label', 'exercise_subtype', 'lights_on', 'extra_person_in_cam18', 'mocap_erroneous'
    ]
    ...
    # Perform mismatches checking
    warnings_list = []
    if N_actual != N_expected:
        warnings_list.append(f"WARNING: Expected {N_expected} manifest rows, but got {N_actual}!")
    if len(unique_subjects) != 9:
        warnings_list.append(f"WARNING: Expected 9 unique subjects, but got {len(unique_subjects)}!")
    ...
    if C_actual != 72:
        warnings_list.append(f"WARNING: Expected 72 correct reps, but got {C_actual}!")
    if I_actual != 26:
        warnings_list.append(f"WARNING: Expected 26 incorrect reps, but got {I_actual}!")

    for warn in warnings_list:
        print(warn, file=sys.stderr)

    # Save manifest
    manifest_csv_path = metadata_dir / "rehab24_squat_sagittal_manifest.csv"
    df_manifest.to_csv(manifest_csv_path, index=False)
    print(f"Manifest saved to: {manifest_csv_path.as_posix()}")

    # 3. MediaPipe Pose Extraction per Video (Stage 2)
    # We run extraction for each video. To display P and F in the sanity checkpoint,
    # we can run the calculations in-memory, log the progress, compute rep trajectories,
    # print the checkpoint, and then write all results to disk.
            ...
            while True:
                ...
                if result.pose_landmarks and len(result.pose_landmarks) > 0:
                    ...
                    if left_complete:
                        ...
                        angle_status = 'valid'
                        knee_angle_deg = compute_knee_angle(landmarks[23], landmarks[25], landmarks[27])
                    else:
                        # Fallback to right side
                        visible_side_used = 'right'
                        if right_complete:
                            angle_status = 'valid'
                            knee_angle_deg = compute_knee_angle(landmarks[24], landmarks[26], landmarks[28])
                        else:
    ...
    # 4. Trajectory Slicing, Smoothing, and Biomarker Extraction (Stages 3-4)
    # We do all calculations in memory first to get success/failed counts for the checkpoint.
    rep_smoothed_dfs = {}
    rep_biomarkers = []
    rep_processing_log = []
    ...
    for idx, row in df_manifest.iterrows():
        ...
        # Modal visible side used
        mode_side = df_rep['visible_side_used'].mode()
        visible_side_used_modal = mode_side.iloc[0] if not mode_side.empty else 'none'
        ...
        res_row = {
            ...
            'visible_side_used': visible_side_used_modal
        }
        ...
        print(f"  [rep {idx+1}/{N_actual}] {vid_id} rep {rep_num}: ROM={log_rom}° peak={log_peak}° tempo={log_tempo} correctness={correctness}")
    ...
    # 5. Pre-write Sanity Checkpoint
    # Print the exact required block to stdout
    print("\n=== Phase 5A REHAB24-6 Integration — Sanity Checkpoint ===")
    print(f"Sagittal squat manifest rows                    : {N_actual}      (expected: 98)")
    print(f"Unique subjects                                 : {len(unique_subjects)}      (expected: 9)")
    print(f"Unique videos                                   : {len(unique_videos)}      (expected: 9)")
    print(f"Videos found on disk                            : {V_count}      (expected: 9)")
    print(f"Reps with correctness=1 (correct form)          : {C_actual}      (expected: 72)")
    print(f"Reps with correctness=0 (incorrect form)        : {I_actual}      (expected: 26)")
    print(f"Reps processed successfully                     : {P_count}")
    print(f"Reps failed (phase_identification_status)       : {F_count}")
    print("==========================================================")

    # Double check videos found on disk
    if V_count < 9:
        sys.exit("STOP: Videos found on disk < 9! Missing video files.")

    print("\nSanity checkpoint check complete. Writing all output files...")

    # Write per-video pose CSV (Stage 2)
    for vid_id, df_pose in video_poses.items():
        pose_csv = pose_dir / f"{vid_id}_pose_full.csv"
        df_pose.to_csv(pose_csv, index=False)
        
    # Write per-rep smoothed CSV (Stage 3)
    for (vid_id, rep_num), df_smoothed in rep_smoothed_dfs.items():
        smoothed_csv = smoothed_dir / f"{vid_id}_rep_{rep_num:02d}_smoothed.csv"
        df_smoothed.to_csv(smoothed_csv, index=False)
        
    # Write combined biomarkers CSV (Stage 5)
    ...
    biomarkers_csv_path = biomarkers_dir / "rehab24_squat_per_rep_biomarkers.csv"
    df_biomarkers.to_csv(biomarkers_csv_path, index=False)
    print(f"Combined biomarkers saved to: {biomarkers_csv_path.as_posix()}")

    # Write per-rep processing status log (Stage 5)
    ...
    proc_log_csv_path = metadata_dir / "rehab24_squat_processing_log.csv"
    df_proc_log.to_csv(proc_log_csv_path, index=False)
    print(f"Processing log saved to: {proc_log_csv_path.as_posix()}")

    # Write per-rep visualizations (Stage 6)
    print("\nGenerating per-rep visualizations...")
    for idx, row in df_manifest.iterrows():
        vid_id = row['video_id']
        sub_id = int(row['subject_id'])
        rep_num = int(row['rep_number'])
        ...
        df_smoothed = rep_smoothed_dfs[(vid_id, rep_num)]
        res = df_biomarkers[(df_biomarkers['video_id'] == vid_id) & (df_biomarkers['rep_number'] == rep_num)].iloc[0]
        
        # 12x4 inches overlay plot
        fig, ax = plt.subplots(figsize=(12, 4))
        
        # Plot smoothed curve
        ax.plot(df_smoothed['frame_index_in_rep'], df_smoothed['knee_angle_smoothed'], color='red', linewidth=2.0, label='Smoothed knee angle')
        
        if res['phase_identification_status'] == 'ok':
            peak_f_frame = int(res['peak_flexion_frame_in_rep'])
            first_non_nan = int(df_smoothed['knee_angle_smoothed'].first_valid_index())
            last_non_nan = int(df_smoothed['knee_angle_smoothed'].last_valid_index())
            ...
            y_min = df_smoothed['knee_angle_smoothed'].min()
            y_max = df_smoothed['knee_angle_smoothed'].max()
            ...
            ax.text(peak_f_frame, y_pos, f"Peak flexion: {res['peak_flexion_deg']:.2f}°", 
        ...
        ax.set_title(f"{vid_id} (Subject {sub_id}) - Rep {rep_num} | ROM: {title_rom} | Peak: {res['peak_flexion_deg']:.2f}° | Tempo: {title_tempo} | correctness: {correctness_str}")
    ...
    summary_txt = f"""Phase 5A REHAB24-6 Integration Summary
======================================
Total reps in manifest: {N_actual} (expected: 98)
...
"""
    ...
    # Save summary report
    summary_txt_path = metadata_dir / "phase5a_integration_summary.txt"
    ...
    print(f"Phase 5A completed successfully.")
    print("============================\n")
